...
var change = function() {
    ...
    var source = "data/per_grant_type_fund/" + yearPart + endPart
...
var svg1 = d3.select("#chart1").append("svg")
  ...
  x.domain(data.map(function(d) { return d.grant_type; }));
  ...
  svg1.selectAll(".bar")
    ...
    .enter().append("rect")
      .attr("class", "bar")
      .attr("x", function(d) { return x(d.grant_type); })
      ...
      .attr("height", function(d) { return height - y(d.fiscal_year_total_cost); })
        .on("mousemove", function(d){tooltip
              ...
              .html((d.grant_type) + "<br>" + "$" + (d.fiscal_year_total_cost));
        })
    ...
    d3.csv(source, function(error, data) {
        ...
        var path = svg2.selectAll('path')
            ...
            .enter()
                .append('path')
                    .attr('d', arc)
                    .attr('fill', function(d) { return color(d.data.grant_type); })
        ...
        path.on('mouseover', function(d) {     
            ...
            tooltip.select('.name').html(d.data.grant_type);           
...
d3.csv("data/top_grant_types.csv", function(error, data) {
    data.forEach(function(d) {
    ...
    // Nest data
    var nested = d3.nest()
        .key(function(d) {return d.grant_type;})
        .entries(data);
            ...
            .data(data)
                ...
                .on("mouseover", function(d) {
                    div.transition()
                        .duration(80)
                        .style("opacity", 1);
                    div.html(d.fiscal_year + "<br/>" + "$" + d.fiscal_year_total_cost + "<br/>" + d.grant_type)
